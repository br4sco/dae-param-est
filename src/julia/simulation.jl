...
function pendulum_sensitivity_Lk(Φ::Float64, u::Function, w::Function, θ::Array{Float64, 1})::Model
    let m = θ[1], L = θ[2], g = θ[3], k = θ[4]
        ...
        function f!(res, xp, x, θ, t)
            ...
            res[8]  = xp[8] - x[11] + 2x[8]xp[6] + 2xp[13]x[1]
            res[9]  = xp[9] - x[12] + 2x[9]xp[6] + 2xp[13]x[2]
            res[10] = 2k*x[11]*abs(x[4]) + m*xp[11] - x[8]xp[3] - xp[10]x[1]
            res[11] = 2k*x[12]*abs(x[5]) + m*xp[12] - x[9]xp[3] - xp[10]x[2]
            ...
            res[17] = abs(x[4])*x[4] + 2k*x[18]*abs(x[4]) - x[1]*xp[17] + m*xp[18] - x[15]*xp[3]
            res[18] = abs(x[5])*x[5] + 2k*x[19]*abs(x[5]) - x[2]*xp[17] + m*xp[19] - x[16]*xp[3]
...
function pendulum_sensitivity_sans_g_with_dist_sens_2(Φ::Float64, u::Function, w::Function, θ::Array{Float64, 1})::Model
    let m = θ[1], L = θ[2], g = θ[3], k = θ[4]
        ...
        function f!(res, xp, x, θ, t)
            ...
            res[30] = 2xp[6]x[30] - x[33] + xp[30] + 2x[2]xp[34]
            res[31] = -xp[3]x[29] + 2k*abs(x[4])*x[32] - x[1]*xp[31] + m*xp[32] - 2*wt[1]*wt[2]
            ...
            res[38] = -xp[3]x[36] + 2k*abs(x[4])*x[39] - x[1]*xp[38] + m*xp[39] - 2*wt[1]*wt[3]
        ...
        xp0 = vcat(pendp0, smp0, sLp0, skp0, rp0, rp0)
        # x0 = vcat([x1_0, x2_0], zeros(4), [atan(x1_0 / -x2_0)], zeros(14))
        # xp0 = vcat([0., 0., dx3_0, dx4_0], zeros(17))
...
function pendulum_sensitivity_sans_g_with_dist_sens_1(Φ::Float64, u::Function, w::Function, θ::Array{Float64, 1})::Model
    let m = θ[1], L = θ[2], g = θ[3], k = θ[4]
        ...
        xp0 = vcat(pendp0, smp0, sLp0, skp0, rp0)
        # x0 = vcat([x1_0, x2_0], zeros(4), [atan(x1_0 / -x2_0)], zeros(14))
        # xp0 = vcat([0., 0., dx3_0, dx4_0], zeros(17))
...
function pendulum_sensitivity_Lk_with_dist_sens_2(Φ::Float64, u::Function, w_comp::Function, θ::Array{Float64, 1})::Model
    let m = θ[1], L = θ[2], g = θ[3], k = θ[4]
        ...
        xp0 = vcat(pendp0, tp0, sp0, rp0, rp0)
        # x0 = vcat([x1_0, x2_0], zeros(4), [atan(x1_0 / -x2_0)], zeros(14))
        # xp0 = vcat([0., 0., dx3_0, dx4_0], zeros(17))
...
function trivial_model(Φ::Float64, u::Function, w_comp::Function, θ::Array{Float64, 1})::Model
    let m = θ[1], L = θ[2], g = θ[3], k = θ[4]
        ...
        function f!(res, xp, x, θ, t)
            ...
            ut = u(t)
            # Dynamic equation
            res[1] = xp[1] + x[1] - wt[1]^2
            # First sensitivity equation
            res[2] = xp[2] + x[2] - 2*wt[1]*wt[2]
            # Second sensitivity equation
            res[3] = xp[3] + x[3] - 2*wt[1]*wt[3]
            nothing
        end
