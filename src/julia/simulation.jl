...
function pendulum_sensitivity_k_with_dist_sens_3(Φ::Float64, u::Function, w::Function, θ::Array{Float64, 1})::Model
    let m = θ[1], L = θ[2], g = θ[3], k = θ[4]
        ...
        dx0 = vcat(pendp0, skp0, rp0, rp0, rp0)
        # x0 = vcat([x1_0, x2_0], zeros(4), [atan(x1_0 / -x2_0)], zeros(14))
        # dx0 = vcat([0., 0., dx3_0, dx4_0], zeros(17))
...
function FORWARD_my_pendulum_adjoint_monly(u::Function, w::Function, θ::Vector{Float64}, T::Float64, x::Function, x2::Function, y::Function, dy::Function, xp0::Vector{Float64}, dx::Function, dx2::Function, N_trans::Int=0)
    # NOTE: A bit ugly to pass sol and sol2 as DAESolution, but dx as a function.
    # But good enough for now, just should be different in final version perhaps
    let m = θ[1], L = θ[2], g = θ[3], k = θ[4]
        np = size(xp0,2)
...
function FORWARD_my_pendulum_adjoint_Lonly(u::Function, w::Function, θ::Vector{Float64}, T::Float64, x::Function, x2::Function, y::Function, dy::Function, xp0::Vector{Float64}, dx::Function, dx2::Function, N_trans::Int=0)
    # NOTE: A bit ugly to pass sol and sol2 as DAESolution, but dx as a function.
    # But good enough for now, just should be different in final version perhaps
    let m = θ[1], L = θ[2], g = θ[3], k = θ[4]
        np = size(xp0,2)
...
# NOTE Assumes free dynamical parameters are only k
function FORWARD_my_pendulum_adjoint_konly(u::Function, w::Function, θ::Vector{Float64}, T::Float64, x::Function, x2::Function, y::Function, dy::Function, xp0::Vector{Float64}, dx::Function, dx2::Function, N_trans::Int=0)
    # NOTE: A bit ugly to pass sol and sol2 as DAESolution, but dx as a function.
    # But good enough for now, just should be different in final version perhaps
    let m = θ[1], L = θ[2], g = θ[3], k = θ[4]
        np = size(xp0,2)
...
function pendulum_adj_stepbystep_L(Φ::Float64, u::Function, w::Function, θ::Array{Float64, 1}, y::Function, λ::Function, dλ::Function, T::Float64)::Model
    ...
    let m = θ[1], L = θ[2], g = θ[3], k = θ[4]
       ...
       #TODO: DON'T PASS T. BUT ALSO DON'T CALL THIS gₓ!!!! EDIT: Why...?
        ...
        dx0  = vcat(pendp0, sp0, dG0, dGp0, dGp02, dGp03, dGp04, dGp04b, dGp035, dpartial, dtue_deb')
...
function pendulum_adj_stepbystep_m(Φ::Float64, u::Function, w::Function, θ::Array{Float64, 1}, y::Function, λ::Function, dλ::Function, T::Float64)::Model
    ...
    let m = θ[1], L = θ[2], g = θ[3], k = θ[4]
       ...
       #TODO: DON'T PASS T. BUT ALSO DON'T CALL THIS gₓ!!!! EDIT: Why...?
        ...
        dGp0  = 2(pend0[7]-y(0.0))*0.0
        Gp02  = 0.0
        dGp02 = dGp0 - (λ(0.)')*(Fx(pend0, pendp0)*sm0 + Fdx(pend0, pendp0)*smp0 + Fp(pend0, pendp0))
        Gp03  = 0.0
        dGp03 = -(λ(0.)')*Fp(pend0, pendp0) - ( (λ(0.)')*(Fx(pend0,pendp0)-Fddx(pend0,pendp0)) -(dλ(0.)')*Fdx(pend0,pendp0) - gₓ(pend0,pendp0,0.0)' )*sm0
        Gp04  = 0.0
        dGp04 = -(λ(0.)')*Fp(pend0, pendp0)
        Gp04b  = 0.0
        dGp04b = - ( (λ(0.)')*(Fx(pend0,pendp0)-Fddx(pend0,pendp0)) -(dλ(0.)')*Fdx(pend0,pendp0) - gₓ(pend0,pendp0,0.0)' )*sm0
        Gp035  = 0.0
        dGp035 = (gₓ(pend0,pendp0,0.0)')*sm0 - (λ(0.0)')*( Fx(pend0,pendp0)*sm0 + Fdx(pend0,pendp0)*smp0 + Fp(pend0,pendp0) )
        partial    = 0.0
...
function mohamed_sens(Φ::Float64, u::Function, w::Function, p::Array{Float64,1})
    ...
    function f!(res, dx, x, p, t)
        ...
        # res[4] = -2*zeta(dx,x,t)*dzeta_dx1(dx,x,t)*x_θ[1] + x_θ[2] -2 *zeta(dx,x,t)*dzeta_dθ(dx,x,t)
        res[3] = dx[3] + θ*x[3] + x[1]
        res[4] = -2*zeta(dx,x,t)*dzeta_dx1(dx,x,t)*x[3] + x[4] - 2*zeta(dx,x,t)*dzeta_dθ(dx,x,t)
        nothing
    end
...
function mohamed_adjoint_new(u::Function, w::Function, p::Array{Float64, 1}, T::Float64, sol::DAESolution, sol2::DAESolution, y::Function, dy::Function, xp0::Array{Float64,2}, dx::Function, dx2::Function)
    θ = p[1]
    np = size(xp0, 2)
    x  = t -> sol(t)
    x2 = t -> sol2(t)
    ...
    function f!(res, dz, z, θ, t)
        ...
        # res[2] = dβ + (λ')*Fθ
        res[1:2] = -(dz[1:2]')*Fdx(T-t) + (z[1:2]')*(Fddx(T-t)-Fx(T-t)) + gₓ(T-t)'
        res[3] = dz[3] + (z[1:2]')*Fθ(T-t)
        nothing
    end
