...
function get_fit_sens(Ye, pars, model, jacobian_model, lb, ub)
    # Use this line if you are using the original LsqFit-package
    return curve_fit(model, jacobian_model, Float64[], Ye, pars, lower=lb, upper=ub, show_trace=true, inplace=false, x_tol=1e-8)    # Default inplace = false, x_tol = 1e-8
...
function get_experiment_data(expid::String)::Tuple{ExperimentData, Array{InterSampleWindow, 1}}
    ...
    # --------------------------------------------------------------------------
    # NOTE: Not fully supported at the moment, so just leave it like it is for now.
    # Working on finalizing code for estimating disturbance parameters on other branches
    # --------------------------------------------------------------------------
...
function write_results_to_file(path::String, opt_pars_baseline, opt_pars_proposed, avg_pars_proposed, trace_proposed, trace_gradient, durations)
...
function debug_minimization(expid::String, pars0::Array{Float64,1}, N_trans::Int = 0)
    ...
    return opt_pars_baseline, opt_pars_proposed, avg_pars_proposed, trace_proposed, base_cost_vals, prop_cost_vals, trace_costs, base_par_vals, prop_par_vals, grad_trace
end
...
function debug_minimization_2pars(expid::String, pars0::Array{Float64,1}, N_trans::Int = 0)
    ...
    return opt_pars_baseline, opt_pars_proposed, trace_proposed, base_cost_vals, prop_cost_vals, base_par_vals, prop_par_vals, base_cost_true, prop_cost_true
end
