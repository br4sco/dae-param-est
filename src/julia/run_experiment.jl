...
include("models.jl")
using .NoiseGeneration: DisturbanceMetaData, demangle_XW, get_ct_disturbance_model, discretize_ct_noise_model_disc_then_diff, simulate_noise_process, simulate_noise_process_mangled
using .NoiseGeneration: discretize_ct_noise_model_with_adj_SDEApprox_mats, discretize_ct_noise_model_with_adj_sensSDEApprox_mats, discretize_ct_noise_model_with_adj_sensSDEApprox_mats_Ainvertible
...
function get_experiment_data(expid::String; use_exact_interp::Bool = false, E_gen::Integer = 100)::Tuple{ExperimentData, Array{InterSampleWindow, 1}}
    ...
    u = if size(U_meta_raw, 2) == 3
        ...
        linear_interpolation_multivar(input, U_meta_raw[1,8], n_u_out)   # U_meta_raw[1,8] is δ used for input signal. Usually same as W_meta.δ, but doesn't need to be.
    end
...
function get_proposed_estimates(pars0::Vector{Float64}, exp_data::ExperimentData, isws::Vector{InterSampleWindow}; 
    use_exact_interp::Bool = false, maxiters::Int = maxiters_opt, verbose::Bool = true, E_in::Int=typemax(Int), method_type::Int = FOR_SENS, Ainvertible::Bool = false, disc_then_diff::Bool = false)
    ...
    let N = size(exp_data.Y, 1)÷md.ny-1, E = min(size(exp_data.Y, 2), E_in), W_meta = exp_data.W_meta, δ = W_meta.δ, Ts = exp_data.Ts, Tsλ = exp_data.Ts/10
        ...
        get_gradient_func = if method_type == FOR_SENS
            ...
            function get_gradient_estimate_for(y::Vector{Float64}, free_pars::Vector{Float64}, isws::Vector{InterSampleWindow}, M_mean::Int=1)
                ...
                dmdl::DT_SS_Model = if disc_then_diff
                    # Discretizes the disturbance model before differentiating wrt to discretization (not recommended)
                    ...
                    discretize_ct_noise_model_disc_then_diff(get_ct_disturbance_model(η, W_meta.nx, W_meta.nv), δ, W_meta.free_par_inds)
                else
                    # Differentiates the disturbance model wrt parameters before discretization (recommended)
                    ...
                    Zm = [randn(W_meta.nx*W_meta.nv*(1+na), W_meta.Nw+1) for _ = 1:2M_mean]
                    η = W_meta.get_all_ηs(free_pars[md.dθ+1:end])  # NOTE: Assumes that the disturbance parameters always come after the dynamical parameters.
                    discretize_ct_noise_model_diff_then_disc(get_ct_disturbance_model(η, W_meta.nx, W_meta.nv), δ, W_meta.free_par_inds)
                end
        ...
        elseif method_type == ADJ_SENS || ((method_type == ADJ_ODEDIST || method_type == ADJ_FORODEDIST) && length(W_meta.free_par_inds)==0)
        ...
        elseif method_type == ADJ_ODEDIST || method_type == ADJ_FORODEDIST
            ...
            function get_gradient_estimate_adjodedist(y::Vector{Float64}, free_pars::Vector{Float64}, isws::Vector{InterSampleWindow}, M_mean::Int=1)
                ...
                η = W_meta.get_all_ηs(free_pars[md.dθ+1:end])  # NOTE: Assumes that the disturbance parameters always come after the dynamical parameters.
                na = length(findall(W_meta.free_par_inds .<= W_meta.nx))   # Number of the disturbance parameters that corresponds to A-matrix. Rest will correspond to C-matrix
                dmdl, Ǎηa, B̌ηa, Čηc, Ǎ = if method_type == ADJ_ODEDIST
                    if Ainvertible
                ...
                else # 'else' is the same as 'elseif ADJ_FORODEDIST' in this case
                    # The returned matrices are, in this case, labeled Ǎηa, B̌ηa, Čηc, Ǎ, even though this is not
                    # consistent with the notation for FORODEDIST case used in my licentiate thesis. However, 
                    # these names are used here nevetheless, because that allows us to share code between ODEDIST and FORODEDIST
                    # If the notation from my licentiate thesis were used instead, the following changes in notation would be made:
                    #  Ǎηa -> Aηa
                    #  B̌ηa -> B̌̌[nxw+1:end, 1:end]
                    #  Čηc -> Cηc
                    #  Ǎ   -> A
                    if Ainvertible
                        discretize_ct_noise_model_with_adj_sensSDEApprox_mats_Ainvertible(get_ct_disturbance_model(η, W_meta.nx, W_meta.nv), δ, W_meta.free_par_inds)
                    else
                        discretize_ct_noise_model_with_adj_sensSDEApprox_mats(get_ct_disturbance_model(η, W_meta.nx, W_meta.nv), δ, W_meta.free_par_inds)
                    end
                end
                ...
                vm(m::Int) = if method_type == ADJ_ODEDIST
                    t -> begin
                        # n*δ <= t <= (n+1)*δ
                        n = Int(t÷δ)
                    ...
                    # In the case of ADJ_FORODEDIST, the white noise input to the ODE has additional inputs compared to the non-sensitivity DT disturbance model
                    Zm_extra = [randn(na*W_meta.nx*W_meta.nv, W_meta.Nw+1) for _ = 1:2M_mean]
                    t -> begin
                        # n*δ <= t <= (n+1)*δ
                        n = Int(t÷δ)
                        vcat(Zm[m][:, n+1], Zm_extra[m][:, n+1])  # +1 because t = 0 (and thus n=0) corresponds to index 1
                    end
                end
