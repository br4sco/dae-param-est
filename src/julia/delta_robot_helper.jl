function delta_robot_f!(res, dz, z, θ, t)
    ...
    # WARN: ONLY A PLACEHOLDER
    u = zeros(3)
    # TODO: ACTUALLY UPDATE WITH CORRECT u
    b = [1.; 0.; 0.]
    ...
    # tau = HˆT(q) * κ + b*u
    tau1 = transpose(dpsi_1)*κ[1:3] + transpose(dpsi_1)*κ[4:6] + b*u[1]
    tau2 = -transpose(Rz(2*pi/3)*dpsi_2)*κ[1:3] + b*u[2]
    tau3 = -transpose(Rz(-2*pi/3)*dpsi_3)*κ[4:6] + b*u[3]
...
function get_delta_initial_dv(θ)
     L0 = θ[1]
     L1 = θ[2]
     L2 = θ[3]
     L3 = θ[4]
     ...
     # TODO CAN*T DEFINE TAU YET, WHICH WE'VE DONE ABOVE
     M = blockdiag(mvec1,mvec2, mvec3)
     CG = vcat(cg1, cg2, cg3)
     B = blockdiag(b,b,b)
     temp1 = (M\(CG - B*u))
     temp2 = ( H*(M\(CG - B*u)) )
     temp3 = (H*(M\transpose(H)))
