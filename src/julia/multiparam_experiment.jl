...
try
    write_custom(exp_id, "opt_pars.csv", coef(fit))
catch e
...
try
    write_custom(exp_id, "true_pars.csv", [θ0; η0])
catch e
...
try
    write_custom(exp_id, "init_pars.csv", [θ0+diff_θ; η0+diff_η])
catch e
...
try
    write_custom(exp_id, "dx_trace.csv", dx_trace)
catch e
