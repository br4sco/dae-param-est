...
function pend_model()::Model
    ...
    function f!(res, xp, x, θ, t)
        res[1] = xp[1] - x[3] + 2*xp[5]*x[1]
        res[2] = xp[2] - x[4] + 2*xp[5]*x[2]
    ...
    x0 = [L*sin(Φ), -L*cos(Φ), 0, 0, 0]
    xp0 = [0, 0, 0, -g, 0]

    dvars = [true, true, true, true, false]
...
function resrob(tres, yy_nv, yp_nv, rr_nv, user_data)
    ...
    # rr = res(yy, yp)
    rr[1] = yp[1] - yy[3] + 2*yp[5]*yy[1]
    rr[2] = yp[2] - yy[4] + 2*yp[5]*yy[2]
...
function res(yy, yp)
    res = zeros(length(yy))
    res[1] = yp[1] - yy[3] + 2*yp[5]*yy[1]
    res[2] = yp[2] - yy[4] + 2*yp[5]*yy[2]
    res[3] = m*yp[3] + k*abs(yy[3])*yy[3]
    res[4] = m*yp[4] + k*abs(yy[4])*yy[4] + m*g
    res[5] = yy[1]^2 + yy[2]^2 - L^2
    return res
end
...
dlow5 = get_derivative_estimate(low5, yp0[5], Ts)
...
high5 = [sol.u[i][5] for i=1:length(sol.u)]
...
dhigh5 = get_derivative_estimate(high5, yp0[5], Ts)

# Stores residual values for low-level method
lowres = zeros(length(low4), 5)
# Stores residual values for high-level method
highres = zeros(length(low4), 5)

for i=1:length(low4)
    yylow = [low1[i], low2[i], low3[i], low4[i], low5[i]]
    yyhigh = [high1[i], high2[i], high3[i], high4[i], high5[i]]
    yplow = [dlow1[i], dlow2[i], dlow3[i], dlow4[i], dlow5[i]]
    yphigh = [dhigh1[i], dhigh2[i], dhigh3[i], dhigh4[i], dlow5[i]]
