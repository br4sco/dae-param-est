...
delta_model_data = let
    ...
    get_all_pars(pars::Vector{Float64}) = [L0, L1, L2, L3, LC1, LC2, M1, M2, M3, J1, J2, g, pars[1]]#vcat(pars[1:11], [g], pars[12])#[L0, L1, L2, L3, LC1, LC2, M1, M2, M3, J1, J2, g, γ]#Array{Float64}(undef, 0)
    ...
    model_sens = delta_forward_γ                        # For forward sensitivity
    model_adjoint = delta_adjoint_γ              # For adjoint sensitivity
    model_adjoint_odedist = delta_adjoint_γ             # Adjoint when disturbances are given by an ODE incorporated into the DAE
    ...
    function get_sens_init(θ::Vector{Float64}, u0::Vector{Float64}, w0::Vector{Float64})::Matrix{Float64}
        ...
        # hcat(zL0, zL1, zL2, zL3, zLC1, zLC2, zM1, zM2, zM3, zJ1, zJ2, zγ, repeat(z0dist,1,12))    # NOTE: Doesn't contain model output, i.e. wrong!!!
