...
function perform_ADAM(
    ...
    let t=1, pars=pars0, trace=[pars0], grad_trace=[]
        ...
        while t <= maxiters
            ...
            rhat = r/(1-betas[2]^t)
            unscaled_step = -shat./(sqrt.(rhat))
            step = learning_rate(t, norm(grad_est)).*unscaled_step
            if verbose
                println("Iteration $t, -gradient $(-grad_est) and step $(step) with parameter estimate $pars")
            end
...
function perform_ADAM_deltaversion(
    get_grad_estimate::Function,
    learning_rate::Function,
    M_rate::Function,
