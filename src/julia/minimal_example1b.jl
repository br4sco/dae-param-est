...
function pend_model()::Model
    ...
    function f!(res, xp, x, θ, t)
        res[1] = xp[1] - x[3]
        res[2] = xp[2] - x[4]
        res[3] = m*xp[3] + k*abs(x[3])*x[3] + sin(t)
...
function resrob(tres, yy_nv, yp_nv, rr_nv, user_data)
    ...
    rr[1] = yp[1] - yy[3]
    rr[2] = yp[2] - yy[4]
    rr[3] = m*yp[3] + k*abs(yy[3])*yy[3] + sin(tres)
...
function res(t, yy, yp)
    res = zeros(length(yy))
    res[1] = yp[1] - yy[3]
    res[2] = yp[2] - yy[4]
    res[3] = m*yp[3] + k*abs(yy[3])*yy[3] + sin(t)
...
for i=1:length(low4)
    ...
    lowres[i,:] = res(times[i], yylow, yplow)
    highres[i,:] = res(times[i], yyhigh, yphigh)
end
